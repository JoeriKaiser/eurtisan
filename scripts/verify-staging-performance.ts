...
async function runProbes() {
  ...
  try {
    ...
    if (anonRes.status === 403) {
      console.warn(
        `[verify-staging-performance] Notice: ${targetUrl} returned 403 Forbidden (Traefik IP whitelist active).`,
      )
      console.warn(
        'To verify against live staging, execute from a whitelisted IP or provide a local URL via PERF_TARGET_URL.',
      )
    ...
    const isPublicCache =
      anonCacheControl.includes('public') && anonCacheControl.includes('s-maxage')
    results.push({
    ...
    const hasVary =
      anonVary.toLowerCase().includes('cookie') &&
      anonVary.toLowerCase().includes('accept-encoding')
    results.push({
    ...
    const isPrivateNoStore =
      authCacheControl.includes('no-store') || authCacheControl.includes('private')
    results.push({
    ...
    console.warn(
      `[verify-staging-performance] Warning: Unable to reach ${targetUrl} (${error.message}).`,
    )
