    ...
    try {
      if (isSignUp) {
        ...
        if (result.error) {
          ...
          // If email verification is required, Better Auth will set emailVerified: false.
          // In that case, we should redirect to verify-email route.
          // Wait, result.data could contain the user status or session status.
          await router.invalidate()

          // Better Auth emailVerification configuration will return verification status.
          // Since requireEmailVerification is set, users need to verify first.
