...
| Category browsing | Populated and empty axe scans; sort group naming, pressed-state, and the two distinct empty states | `not-run` | Verify the breadcrumb trail, subcategory grid, sort/price/in-stock controls, and that the ranking disclosure is reachable and readable as a disclosure widget. |
| Reviews | Review-disclosure and report-dialog axe scans; radio-group naming, required-field, busy-state, and error-announcement tests | `not-run` | Verify the disclosure opens and reads as a disclosure widget, the report dialog traps focus and restores it to the flag button, the reason group is announced as a group, and the required explanation is announced when "something else" is chosen. |
