...
When one page is composed of several sizeable panels, group them in a named subfolder beside the page rather than flattening them into the route folder or promoting them to `src/components/`. `src/route-components/shops/storefront/` is the reference: `ShopStorefront.tsx` composes `ShopBanner`, `ShopIdentityHeader`, `ShopStoryPanel`, `ShopPoliciesPanel`, `ShopSocialLinks`, and `ShopProductFilters`, none of which is used by another route. A panel that later gains a second consumer graduates to `src/components/`; a genuine primitive extracted from one — as `StarRating` was — goes to `src/components/ui/`.
